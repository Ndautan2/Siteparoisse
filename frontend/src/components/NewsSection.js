    ...
    <section className="py-20 bg-gradient-to-b from-white via-white to-slate-50/50 overflow-hidden" data-testid="news-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-14">
          ...
          <h2 className="font-serif text-4xl md:text-5xl font-normal tracking-tight text-slate-deep dark:text-slate-100 mb-4" data-testid="news-title">
            Actualités
          </h2>
        ...
        {news.length === 0 ? (
          <p className="text-center text-slate-500 dark:text-slate-400" data-testid="no-news">Aucune actualité pour le moment.</p>
        ) : (
          <div 
            className="relative"
          ...
          >
            ...
            <div className="overflow-hidden px-2">
              ...
              >
                {news.map((item) => (
                  ...
                  >
                    <div className="bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-500 h-full flex flex-col border border-slate-100/80 cursor-pointer" onClick={() => setSelectedArticle(item)}>
                      {/* Image */}
                      ...
                      <div className="p-5 flex flex-col flex-grow">
                        ...
                        {/* Title */}
                        <h3 className="font-serif text-xl text-slate-deep dark:text-slate-100 mb-2 group-hover:text-gold transition-colors duration-300 line-clamp-2" style={{ overflowWrap: 'break-word', wordBreak: 'normal' }}>
                          {item.title}
                        </h3>

                        {/* Content Preview */}
                        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed mb-4 line-clamp-3 flex-grow" style={{ wordBreak: 'keep-all', overflowWrap: 'break-word' }}>
            ...
            {news.length > 3 && (
              <>
                <button
                  onClick={prevSlide}
                  className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 lg:-translate-x-4 w-11 h-11 bg-white rounded-full shadow-lg flex items-center justify-center text-slate-600 dark:text-slate-400 hover:text-gold hover:shadow-xl transition-all duration-300 z-10 border border-slate-100 dark:border-slate-700 dark:border-slate-700"
                ...
                <button
                  onClick={nextSlide}
                  className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 lg:translate-x-4 w-11 h-11 bg-white rounded-full shadow-lg flex items-center justify-center text-slate-600 dark:text-slate-400 hover:text-gold hover:shadow-xl transition-all duration-300 z-10 border border-slate-100 dark:border-slate-700 dark:border-slate-700"
      ...
      {selectedArticle && (
        ...
        >
          <div
            className="bg-white dark:bg-slate-800 sm:rounded-2xl rounded-t-2xl shadow-2xl max-w-3xl w-full max-h-[85vh] sm:max-h-[90vh] flex flex-col"
            ...
            <div className="flex-shrink-0 sm:rounded-t-2xl rounded-t-2xl overflow-hidden relative">
              {(selectedArticle.image_url || DEFAULT_CATEGORY_IMAGES[selectedArticle.category]) ? (
                ...
                <div className="p-4 sm:p-6 bg-gradient-to-r from-gold/20 to-gold/10">
                  ...
                  <h2 className="font-serif text-xl sm:text-2xl text-slate-deep dark:text-slate-100 leading-tight">
                    {selectedArticle.title}
                  </h2>
